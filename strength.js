export class Strength {
  ...
  export class StrengthManager {
    static saveStrengthDetails(strength) {
      ...
      strengthDetails.push({ strength1: strength.strength1, strenght2: strength.strenght2 });
    ...
    static loadStrength() {
      ...
      strengthDetails.forEach((strengthData) => {
        let strength = new Strength(strengthData.strength1, strengthData.strenght2);
    ...
    static removeStrength(strength, cardElement) {
      ...
      strengthDetails = strengthDetails.filter(
        (c) => JSON.stringify(c) !== JSON.stringify({ strength1: strength.strength1, strenght2: strength.strenght2 })
      );
      sessionStorage.setItem("strength", JSON.stringify(strengthDetails));
      cardElement.remove();
    }
